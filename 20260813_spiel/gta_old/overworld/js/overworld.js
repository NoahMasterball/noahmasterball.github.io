...
class OverworldGame {
    ...
    createDynamicAgents() {
        ...
        let spawnPoint = null;
        if (config.spawnPoint && hasFiniteNumber(config.spawnPoint.x) && hasFiniteNumber(config.spawnPoint.y)) {
            const spawnX = Number(config.spawnPoint.x);
            const spawnY = Number(config.spawnPoint.y);
            spawnPoint = {
                x: spawnX,
                y: spawnY,
                wait: hasFiniteNumber(config.spawnPoint.wait) ? Number(config.spawnPoint.wait) : null,
            ...
            if (hasFiniteNumber(left) && hasFiniteNumber(right) && hasFiniteNumber(top) && hasFiniteNumber(bottom)) {
                spawnPoint = {
                    x: (left + right) / 2,
                    y: (top + bottom) / 2,
                    wait: null,
        ...
        const fixedSpawn = (config.fixedSpawn != null) ? Boolean(config.fixedSpawn) : Boolean(spawnPoint);
        ...
        const npc = {
            ...
            spawnPoint: spawnPoint ? { x: spawnPoint.x, y: spawnPoint.y } : null,
        ...
        if (npc.stayOnSidewalks && !npc.fixedSpawn) {
...
OverworldGame.prototype.applySidewalkConstraint = function (npc) {
    if (!npc || !npc.stayOnSidewalks || npc.fixedSpawn) {
        return;
    }
