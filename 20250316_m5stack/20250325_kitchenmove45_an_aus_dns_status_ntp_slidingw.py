...
# ----------------------------------------------------------------------------------------------
# HELFER-FUNKTION: Formatierung des Zeitstempels für Debug-Ausgaben und nanoleaf API Key holen
# ----------------------------------------------------------------------------------------------
    ...
    try:
        with open(filename) as f:
            for line in f:
                ...
                key = key.strip()
                value = value.strip().strip('"').strip("'")  # Entfernt Anführungszeichen
                secrets[key] = value
    except OSError:
...
def pir_aktiv(pir):
    ...
    if now < manuell_override_bis:
        ...
        return
    # Wenn das Licht bereits an ist, soll trotzdem der Zeitstempel aktualisiert werden,
    # aber es darf keine LED-Feedback-Anzeige (Farbverlauf) erfolgen.
    if ueberkopflicht_an():
        if DEBUG:
            print("{} Licht ist an, PIR event detected – Timestamp aktualisiert, kein LED Feedback.".format(format_debug_time(local_time())))
        last_event = now
        pir_active = True
        return
    # Falls das Licht aus ist, erfolgt der bisherige Ablauf inklusive LED-Feedback.
    if DEBUG:
