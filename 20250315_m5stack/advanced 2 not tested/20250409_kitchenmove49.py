...
class LEDController:
    ...
    def update(self):
        """Update LED display, turn off if expired"""
        now = time.time()
        if self.display_active and now >= self.display_expiry:
            self.led_rgb.fill_color(self.config.LED_COLORS["AUS"])
            self.display_active = False
            self.display_expiry = 0
            self.display_color = None
...
class KitchenLightOrchestrator:
    ...
    def setup(self):
        ...
        # Set LED controller reference in main light controller
        self.main_light_controller.led_ctrl = self.led_controller
