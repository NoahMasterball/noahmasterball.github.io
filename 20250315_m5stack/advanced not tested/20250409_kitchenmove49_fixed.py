...
if TESTMODE:
    INAKT_TIMEOUT = 60          # Testmodus: 60 Sek. Inaktivität
...
last_pir_active = None  # Zeitstempel der letzten PIR-Aktivierung (für Inaktivitätsprüfung)
...
def pir_aktiv(pir):
    global pir_events, last_reset, raum_belegt, last_event, last_pir_active, pir_active, manuell_override_bis
    now = time.time()
    
    # Update last PIR active time
    last_pir_active = now
    ...
    # Light is off, accumulate events
    if DEBUG:
        print("{} Bewegung erkannt (PIR) um {}.".format(format_debug_time(local_time()), int(now)))
    
    # Reset events if too much time has passed since last event
    if pir_events and (now - pir_events[-1]) > 30:  # 30 seconds timeout between events
        pir_events = []
        if DEBUG:
            print("{} PIR-Events zurückgesetzt (30s Timeout überschritten)".format(format_debug_time(local_time())))
...
def pir_inaktiv(pir):
    global pir_active, last_event, last_pir_active
    now = time.time()
    
    # Only process inactivity if light is on
    if last_event is not None and ueberkopflicht_an():
        ...
        if DEBUG:
            print("{} PIR meldet keine Aktivität. Kein Bewegungstimer aktiv oder Licht ist aus.".format(format_debug_time(local_time())))
...
def main_loop():
    global wled_auto_aus_timer, wled_status, raum_belegt, last_event, last_pir_active, manuell_override_bis, pir_events
    ...
    try:
        ...
        # --------------------------------------------------------------------------
        # AUTO-OFF BEI INAKTIVITÄT – NUR WENN LICHT AN UND KEIN PIR FÜR INAKT_TIMEOUT
        # --------------------------------------------------------------------------
        if last_event is not None and last_pir_active is not None:
            # Check if we haven't received any PIR activity for the timeout period
            if (now - last_pir_active) >= INAKT_TIMEOUT:
                if DEBUG:
                    print("{} Inaktivität erkannt ({} Sek. seit letzter PIR-Aktivität) – schalte Licht aus.".format(format_debug_time(local_time()), int(now - last_pir_active)))
                reagiere_raum_unbelegt()
                last_event = None
                last_pir_active = None
                pir_events.clear()
                raum_belegt = False
