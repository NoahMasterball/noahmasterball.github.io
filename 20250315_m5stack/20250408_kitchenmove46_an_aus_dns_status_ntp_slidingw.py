...
TESTMODE = False  # Testmodus: True; Produktion: False

if TESTMODE:
    INAKTIV_TIMEOUT = 60          # Testmodus: 20 Sek. Inaktivität
    EVENT_THRESHOLD = 5           # Schwellenwert 2 im Testmodus
    MANUAL_OVERRIDE_TIME = 60     # 20 Sek. manueller OV im Testmodus
...
# ------------------------------------------------------------------------------
# .env-Datei einlesen (wie gewünscht)
# ------------------------------------------------------------------------------
...
# Schalte-Funktion, welche den neuen Zustand zurückgibt
...
# Aktualisiere den Lichtzustand-Cache
...
def reagiere_raum_belegt():
    if not ist_dunkel_genug():
        if DEBUG:
            print("{} Raum belegt: Shelly/NL an. (nxt in {} Sek.)".format(format_debug_time(local_time()), MANUAL_OVERRIDE_TIME))
        return
    if DEBUG:
        print("{} Raum belegt: Shelly/NL an. (nxt in {} Sek.)".format(format_debug_time(local_time()), MANUAL_OVERRIDE_TIME))
    setze_shelly("ein")
...
def ueberkopflicht_an():
    ...
    if now - last_state_update_time < refresh_interval:
        ...
        if DEBUG:
            print("{} Zust.-akt. übersprungen ({} Sek. alt), nxt in {} Sek. (Cache: {})".format(format_debug_time(local_time()), int(now - last_state_update_time), remaining, cached_light_state))
        ...
        if DEBUG:
            print("{} Zust.-akt. OK: Shelly={}, NL={}, Res={} - gilt {} Sek.".format(format_debug_time(local_time()), shelly_state, nanoleaf_state, updated_state, refresh_interval))
